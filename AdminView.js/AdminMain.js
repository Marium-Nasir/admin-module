  ...
  const getQueries = async () => {
    try {
      ...
      setQnaData(data[0].QnA);
        setQueryId(data[0]._id)
      // setqnaId(data[0].QnA._id)
      console.log(`qna ${qna}`);
    } catch (err) {
      console.log(err);
    }
...
console.log(`fron admin page ${queryId}`);
console.log(`fron qnaID ${qnaId}`);
console.log(`answer ${answer}`);
  const answerQuery = async()=>{
    try{
      ...
      const {data} = await axios.patch(`/api/user/answerquery?ansQueryId=${queryId}`,requestBody,config);
    }catch(err){
      console.log(err);
    }
  }
